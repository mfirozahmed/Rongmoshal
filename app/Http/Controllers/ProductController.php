...
class ProductController extends Controller
{
    ...
    public function product_specific($id)
    {
        ...
        return view('admin.product_specific')->with('product', $product);
    }
    ...
    public function product_add()
    {
        ...
        return view('admin.product_add')->with('main_categories', $main_categories);
    }
    ...
    public function product_edit($id)
    {
        ...
        return view('admin.product_edit')->with('product', $product)->with('sub_categories', $sub_categories)->with('main_categories', $main_categories)
    ...
    public function top()
    {
        $signature_products = Product::whereIn('top', array(2, 3))->get();
        $new_arrivals = Product::whereIn('top', array(1, 3))->get();
        $most_sell = [];
        ...
        return view('admin.top')->with($data);
    }

    public function top_add($id)
    {
        $products = Product::all();
        return view('admin.top_add')->with('products', $products)->with('id', $id);
    }
    ...
    public function top_add_submit($pid, $type)
    {
        ...
        if ($type == 'signature') {
            if ($product->top == 0) 
                $product->top = 2;
            elseif ($product->top == 1) 
                $product->top = 3;

        } elseif ($type == 'new') {
            if ($product->top == 0) 
                $product->top = 1;
            elseif ($product->top == 2) 
                $product->top = 3;
        }
    ...
    {
        if ($id == 'signature')
            $products = Product::whereIn('top', array(2, 3))->get();
        elseif ($id == 'new')
            $products = Product::whereIn('top', array(1, 3))->get();
        return view('admin.top_remove')->with('products', $products)->with('id', $id);
    }
